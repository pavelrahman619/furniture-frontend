  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        productImages = productData.variants[0].images.map(
      ...
      const variantOptions = productData.variants
        ...
        .map((v: ProductVariant) => ({
          ...
          label: v.attribute || "",
          priceModifier:
            v.price - (productData.variants[0]?.price || productData.price),
        }))
        .filter(
          (v: { value: string }, index: number, self: { value: string }[]) =>
            index ===
            self.findIndex((t: { value: string }) => t.value === v.value)
        );
      ...
      const transformedProduct: ProductDetails = {
        ...
        stockInfo: stockData.locations.map(
          (location: StockResponse["locations"][0]) => ({
        ...
        productLevelImages: productData.images.map(
          (img: ProductImage) => img.url
        ),
